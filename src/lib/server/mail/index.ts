...
class Mailer {
    ...
    async init() {
        ...
        try {
            ...
            if (!PUBLIC_SMTP_HOST || !PUBLIC_SMTP_PORT || !PUBLIC_SMTP_IDENTIFIANT || !SECRET_SMTP_PASSWORD || !PUBLIC_SMTP_SENDER || !PUBLIC_SMTP_SENDER_NAME) {
                logger.error({}, "Erreur lors de la récupération des paramètres de configuration du serveur SMTP")
            }

            if (PUBLIC_SMTP_HOST.value.length <= 0 || PUBLIC_SMTP_PORT.value.length <= 0 || PUBLIC_SMTP_IDENTIFIANT.value.length <= 0 || SECRET_SMTP_PASSWORD.value.length <= 0 || PUBLIC_SMTP_SENDER.value.length <= 0 || PUBLIC_SMTP_SENDER_NAME.value.length <= 0) {
